...
LayoutSlidePart::LayoutSlidePart(opc::OpcPackage& package,
...
void LayoutSlidePart::load() {
    auto content = package_.get_part(part_name_);
    if (!content) {
        throw std::runtime_error(
            "Layout slide part not found: " + part_name_);
    }
    auto result = doc_.load_buffer(content->data(), content->size());
    if (!result) {
        throw std::runtime_error(
            "Failed to parse layout slide XML: " + part_name_);
    }
...
const std::string& LayoutSlidePart::part_name() const {
    return part_name_;
}

std::string LayoutSlidePart::name() const {
...
std::string LayoutSlidePart::type() const {
    auto type_attr = root_.attribute("type");
    if (type_attr) {
...
std::string LayoutSlidePart::master_part_name() const {
    auto rels = rels_manager_.get_relationships_by_type(
        opc::rel_types::kSlideMaster);
    if (rels.empty()) {
        return "";
    }
...
std::string LayoutSlidePart::resolve_target(std::string_view target) const {
