...
std::unordered_map<std::string, ExporterFactory> ExporterRegistry::exporters_;

bool ExporterRegistry::unregister_exporter(std::string_view format_value) {
    auto it = exporters_.find(std::string(format_value));
...
bool ExporterRegistry::is_format_supported(std::string_view format_value) {
    return exporters_.contains(std::string(format_value));
}
